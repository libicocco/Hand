...
**********************************************/
#ifndef __CREATELSHINDEX__H
#define __CREATELSHINDEX__H
#include <cstdlib>
#include <iostream>
...
// void run(const std::string pDataPath,const std::string pIndexPath,const float pW,
    ...
    for (int m = begin_M; m < end_M; ++m) {
        ...
        if (ok) {
            ...
            pW=(MIN_W + DELTA_W * input[3]) * sqrt(scale);
						return; // return with the first successful parameter; another option is to run all and keep the best
        } else {
            cerr << "Failed." << endl;
        }
...
void run(const std::string pDataPath,const std::string pIndexPath,const float pW,
        const unsigned pM,const unsigned pNTables,Index *pIndex,const unsigned pHashSize=1017881)
{
    boost::timer timer;
    ...
    pIndex->init(param, rng, pNTables);
    // The accessor.
