...
public:
    ...
    /// JAVIER:Constructor from data directly
    DataParam (const double gM,const double gG,const gsl_vector *pM, const gsl_vector *pG):
        M(gM),G(gG),
        a_M(gsl_vector_get(pM,0)),b_M(gsl_vector_get(pM,1)),c_M(gsl_vector_get(pM,2)),
        a_G(gsl_vector_get(pG,0)),b_G(gsl_vector_get(pG,1)),c_G(gsl_vector_get(pG,2)){}
